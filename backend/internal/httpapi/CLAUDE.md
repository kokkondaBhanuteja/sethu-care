...
- `admin_*.go` — the admin console's contract (`AdminHandler`, frozen shapes; run `make openapi-check`, never regen-drift it). Phase 1 implemented: shell counters, dashboard summary/attention/activity (`ops` service), bookings list/detail (`booking.AdminList`/`AdminDetailByID` + `ledger.PaymentFactsForBooking` for the payment panel — the one deliberate two-service handler, since booking must not read the ledger), audit list (`audit.List`). Phase 2 implemented the ALERT family (`admin_alerts.go` → `alert.Service`; wire camelCase ↔ domain UPPER_SNAKE mapped here; alert ids also resolve a booking id per the phase-1 `alertId = booking id` convention; acknowledge/notes honour Idempotency-Key), the dashboard band + the now-real `criticalAlerts` shell counter (`alert.Band`/`CountOpenCritical`), the live map (`admin_map.go` → `ops.LiveMap`; zones/clusters/zeroSupplyZoneIds honestly empty, zoneId always "", viewport params accepted but unused), audit detail + admins (`audit.Get`/`ListAdmins`), AND the booking ACTIONS (`admin_booking_actions.go` → `rescue.Service`): the five `*-context` reads plus cancel/undo-cancel/undo-assign/redispatch/manual-complete/refund, all Idempotency-Key honoured. `adminActionError` builds the DECLARED error bodies (VERSION_CONFLICT 409, TOO_EARLY 409 + availableAt, EVIDENCE_INSUFFICIENT 422 + missing[], EXCEEDS_CAP 422 + fields, RATE_LIMITED 429 + resetAt) as `huma.StatusError` wrappers; every other error still goes through `classify`. The detail endpoint flips `isAdminVerified` + `verification` (48h dispute window) on a `completedByAdmin` timeline entry. Phase 2 also implemented PROVIDERS + APPLICATIONS (`admin_providers.go`/`admin_applications.go` → `providerops.Service`): roster/status derivation, suspend/block/force-offline/restore with version CAS + live-job resolution, and the application pipeline (approve provisions a technician in the same tx). Phase 2 finally implemented the AUTH family (`admin_auth.go` — login/2fa/resend/bootstrap/devices/logout/refresh/unlock over `adminaccount.Service`, bearer minted by the one `Signer` with `identity.RoleAdmin`, OTP delivered like `AuthHandler` does — SMS or dev log, NEVER a response field) and the SETTINGS family (`admin_settings.go` — profile/notifications/security/version/queued-actions/diagnostics). Remaining families (payouts, v1.1) still 501 via `notImplemented`. `adminBookingReference` derives the operator reference (#B- + first 8 id hex digits). CONTRACT QUIRK: `opsListAlerts`'s `acknowledged` is a plain bool, so omitted == explicit false; false returns the WHOLE feed and only `acknowledged=true` narrows (documented at the handler).
