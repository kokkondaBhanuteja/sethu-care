...
export interface AssignRankingPanelProps {
  ...
  /** Open when "no providers" is the claim on screen — this panel is that claim's evidence. */
  defaultOpen?: boolean;
}
 ...
 * front of them at all. Collapsible so the candidate table keeps the modal's full width; it stays
 * rendered through the no-candidates state for the same reason — "no providers" is a claim, and
 * this panel is the evidence for it.
 */
export function AssignRankingPanel({
  weights,
  rounds,
  declinedCount,
  defaultOpen = false,
}: AssignRankingPanelProps) {
  ...
  return (
    <Card tone="surface">
      <button
        ...
        className="flex w-full items-center justify-between gap-s2"
        onClick={() => setIsOpen((wasOpen) => !wasOpen)}
      >
        <span className="text-emph text-text-1">{t("assign.rankingTitle")}</span>
        <Icon glyph={ChevronDown} className={isOpen ? "rotate-180 text-text-2" : "text-text-2"} />
      </button>
