...
- `ProviderStatusIndicator.tsx` — dot + word for free / on job / offline; a labelled pill for suspended and offboarded, because a standing decision has to be readable as a word. Also exports `AVATAR_STATUS_FOR_PROVIDER`.
- `MetricTile.tsx` — one performance figure, its §6.16 band colour, and its sparkline.
...
**Roster** — `SupplyBanner.tsx` (desktop strip + mobile card), `RosterCountTags.tsx`, `RosterCard.tsx`, `RosterTable.desktop.tsx`, `ApplicationsEntryRow.tsx`.
...
Dependencies: `../../../components/ui`, `../../../lib/{cx,format}`, `../../../routes/routes.constants`, `@sethu/i18n`, lucide-react.
