...
**Applications** — `ApplicationAge.tsx` (ageing pill + completeness), `ApplicationsTable.desktop.tsx` (decided rows fade under an "Already decided" group header inside the pending segment), `ApplicationCard.tsx` (danger edge on the same >5-day threshold as the pill and row tint), `ApplicantCard.tsx`, `ApplicationDocumentList.tsx`, `ApplicationReviewSections.mobile.tsx` (background check lives in `ApplicationFacts` only — no duplicate section), `ApplicationStateBanners.tsx`, `ReviewerNotesCard.tsx` (an explicitly-unsaved draft: no submit button until a notes mutation exists), `RejectApplicationForm.tsx`, `RejectApplicationDialog.tsx` (the commit button says "Reject application", never a bare "Confirm"), `DocumentFilmstrip.desktop.tsx`, `DocumentViewer.desktop.tsx`.
