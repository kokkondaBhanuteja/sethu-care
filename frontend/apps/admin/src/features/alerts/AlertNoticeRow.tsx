 ...
 * ignorable, or the tier above it stops meaning anything (spec §6.20).
 */
export function AlertNoticeRow({ alert, inset = false, showChevron = false }: AlertNoticeRowProps) {
  ...
  return (
    <Link
      ...
      className={cx(
        "flex min-h-row-48 items-center gap-s3 border-b border-border-subtle no-underline",
        inset && "px-s4",
      )}
    >
      ...
      {showChevron ? <Icon glyph={ChevronRight} className="shrink-0 text-text-3" /> : null}
    </Link>
  );
}
