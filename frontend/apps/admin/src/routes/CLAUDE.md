...
- `routes.constants.ts` — `ROUTES` (typed path builders), `ROUTE_PATTERNS` (`:param` forms), `ROUTE_TABLE` (pattern + tab + surface + guarded action), `TABS`, `SURFACES`.
...
- `SurfaceGuard.tsx` — a `desktopOnly` route on a phone renders the "Best on desktop" notice (spec §6.34), never a blank screen or a 404.
