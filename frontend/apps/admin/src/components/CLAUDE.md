...
- `ui/` — the design-system primitives, in two kinds:
  ...
  - **Admin-only composites**, kept app-owned and styled on tokens: Timeline, StepRail, StepUpChallenge, StatusDot, Banner, RecordText, Badge, Panel, Spinner, Tooltip (ui-web has none yet), Pagination (deliberately count + "Load more", NOT the ui-web numbered pager), FilterBar (chip toggles; `filterChipClassName` is the exported chip look), ToastHost (admin toast system stays for now; an action inside a draining window also states its deadline textually — "Undo · 8s" — because the 2px progress line alone is not legible), Icon (the lucide wrapper). The Button adapter's `inline` size lifts to a 44px min tap target below the 768px shell split (`max-md:min-h-11`) while desktop tables keep the compact 32px row control.
...
- `ui/states/` — the §4.10 screen states that are not a plain empty: filtered-empty, not-found, permission-denied, coming-soon. `ComingSoonState` takes an optional `interim` path (hint line + link) so a placeholder names the route to the same answer today instead of dead-ending; pages give it chrome via `layouts/PageFrame`.
