...
**Status: P0, in progress.** The schema, the enum layer, the 13-state booking machine, and
the transactional transition engine (CAS + append-only event + outbox, all in one
transaction) are built and proven against a real database. There is no HTTP API beyond
`/health` yet, and no UI at all.
